...
public interface ContentInfoRepository extends JpaRepository<ContentInfo,Integer> {
     ...
     * @return：文章ID、更新时间、标题
     */
    @Query(value = "SELECT content_id,update_time,title FROM content_info WHERE is_view =1 ORDER BY update_time desc limit 0,5",nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "SELECT content_id,read_num,title FROM content_info WHERE is_view =1 ORDER BY read_num desc limit 0,5",nativeQuery = true)
